...
const Footwear = () => {
  ...
  useEffect(() => {
    const dbRef = ref(db, 'footwears');
    onValue(dbRef, (snapshot) => {
      const data = snapshot.val();
      const fetchedFootwears = [];
      for (let key in data) {
        if (key !== 'latest') {
          fetchedFootwears.push({ key, ...data[key] });
        }
      }
      setFootwears(fetchedFootwears);
      setImages(fetchedFootwears.map(veg => veg.url));
      setLoading(false);
    });
  }, []);
  ...
  const handleDelete = async (key, url) => {
    try {
      await remove(ref(db, `footwears/${key}`));
      const footwearRef = storageRef(storage, `footwears/${url.split('/').pop().split('?')[0]}`);
      await deleteObject(footwearRef);
      setFootwears(footwears.filter(footwear => footwear.key !== key));
    } catch (error) {
          ...
          {footwears.length > 0 ? (
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 llg:grid-cols-3 place-items-center dlg:max-w-[1440px] gap-10 w-full'>
              {footwears.map((footwear, index) => (
                <div key={footwear.key} className='w-full h-[300px] productBox pb-2 cursor-pointer'>
                  <div onClick={() => handleView(index)} className='w-full h-[200px]'>
                    <img src={footwear.url} className='h-full w-full object-contain drop-shadow-2xl' alt="Footwear" />
                  </div>
            ...
            <div className=" grid place-items-center md:flex md:items-center md:gap-5 justify-center items-center">
              ...
              <p className=" font-bold text-center">No Footwear Found</p>
            </div>
          )}
        </div>
      </section>
